...
export async function GET(request: NextRequest) {
  try {
    ...
    const [successRate, avgPostTime, topResorts, recentActivity] =
      await Promise.all([
        prisma.publishAttempt
          .aggregate({
            where: {
              instagramAccountId,
              status: "SUCCESS",
          ...
          .then(async (success) => {
            const total = await prisma.publishAttempt.count({
        ...
        prisma.schedule
          ...
          .then((schedules) => {
            ...
            const totalDelay = schedules.reduce((sum, schedule) => {
              if (schedule.publishedAt && schedule.scheduledFor) {
                return (
                  sum +
                  (new Date(schedule.publishedAt).getTime() -
                    new Date(schedule.scheduledFor).getTime())
                );
              }
              return sum;
            }, 0);
            return totalDelay / schedules.length;
          }),
        prisma.reel.groupBy({
          by: ["resortId"],
          where: {
    ...
    const topPerformingResorts = await Promise.all(
      topResorts
        .filter((group) => group.resortId !== null)
        .map(async (group) => {
